...
export const SessionManagement: React.FC<SessionManagementProps> = ({
  ...
  const getSessionStatus = (session: AttendanceSession) => {
    if (!session.isActive) return { status: 'Not Started', color: 'bg-slate-100 text-slate-500 border-slate-200' };
      ...
      <motion.div variants={itemVars} className="glass-panel p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-900 tracking-tight">Session Architect</h3>
              <p className="text-slate-500 text-sm">Deploy and track biometric endpoints.</p>
            </div>
          </div>
        ...
        {showCreateForm && (
          ...
          >
            <div className="glass-panel p-8 mt-2">
              <h4 className="text-slate-900 font-semibold mb-6 flex items-center"><Activity size={18} className="text-primary mr-2" /> Provision New Environment</h4>
              <form onSubmit={handleCreateSession} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-2">Target Room</label>
                    <select
                      ...
                      required
                    >
                      <option value="" className="bg-white">Select physical/virtual bounding</option>
                      {availableRooms.map(room => (
                        <option key={room.roomId} value={room.roomId} className="bg-white">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-2">Session Subject</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-2">Activation Time</label>
                    <input
                      type="datetime-local"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-2">Termination Target</label>
                    <input
                      type="datetime-local"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-2">Mission Directives</label>
                  <textarea
      ...
      <motion.div variants={itemVars} className="space-y-4 pt-4">
        ...
        {activeSessions.length === 0 ? (
          <div className="glass-panel text-center py-16 px-4">
            ...
            <h4 className="text-xl font-bold text-slate-900 mb-2">Zero Deployments</h4>
            <p className="text-slate-500 mb-6">The sensor network is currently idle.</p>
                ...
                <div key={session.id} className="glass-card p-6 flex flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h4 className="text-xl font-bold text-slate-900 mb-2">{sessionForm.subject || 'Standard Session'}</h4>
                      ...
                      <button onClick={() => setSelectedSession(session)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-400 hover:text-slate-900">
                        <Eye size={20} />
                      </button>
                    </div>

                    <div className="space-y-2.5 bg-slate-50/80 rounded-xl p-4 border border-slate-200/60">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-500">Node Location</span>
                        <span className="text-slate-900 font-mono">{session.roomId}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
          ...
          >
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="glass-panel w-full max-w-2xl overflow-hidden shadow-2xl">
              <div className="p-6 border-b border-slate-200/60 flex items-center justify-between bg-slate-50/80">
                ...
                <button onClick={() => setSelectedSession(null)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-900 transition">
                  ×
                </button>
              </div>
              ...
              <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="bg-slate-50/80 rounded-xl p-4 border border-slate-200/60">
                    ...
                    <p className="text-slate-900 font-bold">{getSessionDuration(selectedSession)}</p>
                  </div>
                  <div className="bg-slate-50/80 rounded-xl p-4 border border-slate-200/60">
                    <p className="text-slate-500 text-xs font-semibold mb-1 uppercase tracking-wider">Sector</p>
                    <p className="text-slate-900 font-bold">{selectedSession.roomId}</p>
                  </div>
                  <div className="bg-slate-50/80 rounded-xl p-4 border border-slate-200/60">
                    <p className="text-slate-500 text-xs font-semibold mb-1 uppercase tracking-wider">Secure Hits</p>
                    <p className="text-slate-900 font-bold">{selectedSession.attendanceRecords?.filter(r => r.status === 'PRESENT').length || 0}</p>
                  </div>
                </div>
